...
# ╔═╡ 425d86e4-fa1f-11f0-bcea-4fafe7356517
using Plots, CSV, DataFrames, Statistics

# ╔═╡ 83a173d7-c502-44be-8a1f-de848b62c9db
const mappings = open("segment_categories.csv", "r") do f
	symbols = String[]
	categories = String[]
	for line in readlines(f)[2:end]
		parts = split(line, ",")
		symb = uppercase(strip(parts[1]))
		cat = strip(parts[2])
		push!(symbols, symb)
		push!(categories, cat)
	end
	Dict(s => c for (s, c) in zip(symbols, categories))
end

# ╔═╡ 6fea0ada-1143-44e7-a399-3dfbd92a5fc4
function create_means()
	res = Dict()
	d = CSV.read("test_audio16_manual_ensemble_model_alignment_results.tsv", DataFrame)
	d.segment = replace.(uppercase.(d.segment), r"[0-9]" => "")
	d.following = [[d.file[i] == d.file[i+1] ? d.segment[i+1] : "#" for i in 1:(nrow(d)-1)]; ["#"]]
	d = filter(:following => !=("#"), d)
	d.cat = [mappings[s] for s in d.segment]
	d.folcat = [mappings[s] for s in d.following]
	d.catseq = d.cat .* " " .* d.folcat
	d.width = d.segment_hi_ci .- d.segment_lo_ci
	c = combine(groupby(d, :catseq), :width => mean)
	c.current = [split(x)[1] for x in c.catseq]
	c.following = [split(x)[2] for x in c.catseq]
	c = c[:,["current", "following", "width_mean"]]
end

# ╔═╡ 6a7e5181-98bc-446f-a19b-28c69fb08d3f
function create_medians()
	res = Dict()
	d = CSV.read("test_audio16_manual_ensemble_model_alignment_results.tsv", DataFrame)
	d.segment = replace.(uppercase.(d.segment), r"[0-9]" => "")
	d.following = [[d.file[i] == d.file[i+1] ? d.segment[i+1] : "#" for i in 1:(nrow(d)-1)]; ["#"]]
	...
	d.catseq = d.cat .* " " .* d.folcat
	d.width = d.segment_hi_ci .- d.segment_lo_ci
	c = combine(groupby(d, :catseq), :width => median)
	c.current = [split(x)[1] for x in c.catseq]
	c.following = [split(x)[2] for x in c.catseq]
	c = c[:,["current", "following", "width_median"]]
end

# ╔═╡ 77a88a9f-7b6e-49d9-b6b0-3b2c98682f4a
d = create_means()

# ╔═╡ e2e53733-8242-496f-821a-e6cdb90e4e7b
m = Array{Union{Float64,Missing},2}(missing, 7, 7)

# ╔═╡ 96862742-84f7-4aef-9f66-9fe51cefe85c
cat2num = Dict(k => i for (i, k) in enumerate(sort(collect(Set(d.current)))))

# ╔═╡ 9604c204-42cd-4ea8-bccf-524e6678081f
for r in eachrow(d)
	i_val = cat2num[r.current]
	j_val = cat2num[r.following]
	m[i_val, j_val] = r.width_mean
end
...
# ╔═╡ bd5abc14-cf8e-4902-84a2-123bfea9e51c
begin
	m2 = Array{Union{Float64,Missing},2}(missing, 7, 7)
	for r in eachrow(md)
		i_val = cat2num[r.current]
		j_val = cat2num[r.following]
		m2[i_val, j_val] = r.width_median
	end
	m2
end
...
manifest_format = "2.0"
project_hash = "1826ce1665d52bbe291abe4c5ea87f6e23628433"
